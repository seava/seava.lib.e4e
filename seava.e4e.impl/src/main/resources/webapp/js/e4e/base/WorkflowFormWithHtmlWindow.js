...
Ext.define("e4e.base.WorkflowFormWithHtmlWindow", {
	...
	constructor : function(config) {
		...
		if (config._wfConfig_.type == "startform") {
			var submitUrl = Main.wfProcessInstanceAPI().start;
		...
		if (config._wfConfig_.type == "taskform") {
			var submitUrl = Main.wfTaskAPI(config._wfConfig_.taskId).complete;
		...
		e4e.base.WorkflowFormWithHtmlWindow.superclass.constructor.call(
				this, cfg);
