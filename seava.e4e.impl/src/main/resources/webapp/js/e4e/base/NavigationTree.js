 ...
 */
Ext.define('e4e.base.NavigationTree$Model', {
...
Ext.define("e4e.base.NavigationTree", {
	...
	initComponent : function(config) {
			...
			listeners : {
				beforeload : {
					...
					fn : function(store, operation, eOpts) {
						store.proxy.extraParams[Main.requestParam.FILTER] = Ext
								.encode({
		...
		this
				.addEvents("openMenuLink", "openMenuLinkInNewTab",
						"refreshCatalog");
	...
	initEvents : function() {
		this.callParent(arguments);
		this.on("itemdblclick", this.onMenuClick, this);
	},

	beforeStoreLoad : function(store, operation, eOpts) {
		if (operation.node.data) {
			store.proxy.extraParams[Main.requestParam.FILTER] = Ext.encode({
