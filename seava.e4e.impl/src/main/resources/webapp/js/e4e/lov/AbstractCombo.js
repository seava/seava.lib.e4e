...
Ext
		.define(
				"e4e.lov.AbstractCombo",
				{
					...
					/**
					 * 
					 * Link to the dc-view fields to filter the records in this
					 * combo.
					 * 
					 * Example: ,filterFieldMapping:
					 * [{lovField:"...lovFieldName", dsField: "...dsFieldName"} ]
					 * 
					 * Or: ,filterFieldMapping: [{lovField:"...lovFieldName",
					 * value: "...static value"} ]
					 */
					filterFieldMapping : null,

					/**
					 * Specify what values should this combo return to the
					 * dc-record.
					 * 
					 * @type Array
					 */
					...
					/**
					 * Data-control view type this field belongs to. Injected by
					 * the corresponding view builder.
					 */
					_dcView_ : null,
					...
					initComponent : function() {
						...
						this
								.on(
										...
										function() {
											if (this.inEditor) {
												this._onFocusValue_ = this._targetRecord_.data[this.column.dataIndex];
											} else {
												this._onFocusValue_ = this
														.getRawValue();
											}
										}, this);
					...
					onTrigger2Click : function() {
						...
						getApplication().showFrame(
								...
								{
									url : Main.buildUiPath(
											this._editFrame_.bundle,
											this._editFrame_.name,
											this._editFrame_.custom),
					...
					_createStore_ : function() {
						if (this._dataProviderName_ == null) {
							...
							this._dataProviderName_ = this.recordModelFqn
									.substring(this.recordModelFqn
											.lastIndexOf('.') + 1,
						...
						this.store = Ext.create("Ext.data.Store", {
							...
							proxy : {
								type : 'ajax',
								api : Main.dsAPI(this._dataProviderName_,
										"json"),
								model : this.recordModel,
								extraParams : {
									params : {}
					...
					_getTargetRecord_ : function() {
						...
						if (this.inEditor
							...
							if (dcv && dcv._dcViewType_ == "bulk-edit-field") {
								/*
								 * is a bulk editor for one ds field in a
								 * property grid
								 */
								drec = dcv.getSource();
							} else {
					...
					/**
					 * Map fields from the combo-record received as argument
					 * (crec) to the target record according to fieldMappings
					 * 
					 * @param {}
					...
					_mapReturnFields_ : function(crec) {
						...
						if (this.inEditor
							...
							if (dcv && dcv._dcViewType_ == "bulk-edit-field") {
								 ...
								 */
								drec = dcv.getSource();
								this._mapReturnFieldsExecuteBulkEdit_(crec,
										drec);
							} else {
								drec = this._targetRecord_;
					...
					_mapReturnFieldsExecuteBulkEdit_ : function(crec, recdata) {
						...
						if (this.retFieldMapping != null) {
							for (var i = this.retFieldMapping.length - 1; i >= 0; i--) {
								...
								var nv = null;
								isParam = !Ext
										.isEmpty(this.retFieldMapping[i]["dsParam"]);
								if (isParam) {
					 ...
					 * The current record or current filter based on the
					 * view-type context <br>
					 * prec - Controller params record
					 */
					_mapReturnFieldsExecute_ : function(crec, drec, prec,
							targetIsFilter) {
						if (!drec) {
							return;
						}
					...
					_mapFilterFields_ : function(bp) {
						...
						if (this.inEditor) {
							drec = this._targetRecord_;
							return this
									._mapFilterFieldsExecute_(bp, drec, prec);
						} else {
							if (dcv._dcViewType_ == "edit-form") {
								drec = dcv._controller_.getRecord();
							}
					 ...
					 * 
					 * drec - Controller data-record. The current record or
					 * current filter based on the view-type context prec -
						...
						if (this.filterFieldMapping != null) {
							...
							for (var i = 0; i < len; i++) {
								...
								if (this.filterFieldMapping[i]["value"] != undefined) {
									...
									if (!Ext
											.isEmpty(this.filterFieldMapping[i]["dsParam"])) {
										_val = prec
												.get(this.filterFieldMapping[i]["dsParam"])
									} else {
										_val = drec
												.get(this.filterFieldMapping[i]["dsField"])
									}
								}
								...
								if (isLovMemberParam) {
									this.params
											.set(
													this.filterFieldMapping[i]["lovParam"],
													_val);
								} else {
					...
					onKeyUp : function(e, t) {
						...
						if (dcv._dcViewType_ == "filter-form"
								|| dcv._dcViewType_ == "filter-propgrid") {
							btc = [ kbs.doEnterQuery, kbs.doClearQuery,
									kbs.doQuery, kbs.doEditOut ];
							var l = btc.length;
							for (var i = 0; i < l; i++) {
								var b = btc[i];
						...
						if (dcv._dcViewType_ == "edit-form"
								|| dcv._dcViewType_ == "edit-propgrid") {
							btc = [ kbs.doNew, kbs.doCopy, kbs.doCancel,
									kbs.doSave, kbs.doEditOut, kbs.nextRec,
									kbs.prevRec, kbs.nextPage, kbs.prevPage ];
							var l = btc.length;
							for (var i = 0; i < l; i++) {
								var b = btc[i];
								if (key == b.key && e.shiftKey == b.shift
					...
					setValue : function(value, doSelect) {
						...
						me.value = me.multiSelect ? processedValue
								: processedValue[0];
