 ...
 */
Ext.define("e4e.dc.tools.DcBulkEditWindow", {
	...
	initComponent : function(config) {
		...
		var btn = Ext.create('Ext.Button', {
			text : Main.translate("dcvgrid", "bulkedit_run"),
			scope : this,
		...
		var cfg = {
			title : Main.translate("dcvgrid", "bulkedit_title"),
			border : true,
			width : 350,
			height : 180,
			resizable : true,
	...
	getField : function(title) {
		var f = this._updatableFields_, l = f.length;
		for ( var i = 0; i < l; i++) {
			if (f[i].title == title) {
				return f[i].name
			}
...
Ext.define("e4e.dc.BulkEditor", {
	...
	getCellEditor : function(record, column) {
		...
		if (record.data.name != "_ds_fieldName_") {
			if (Ext.isEmpty(src._ds_fieldName_)) {
				Ext.Msg.show({
					...
					msg : "Select first the field to update, "
							+ "then try to set its value.",
		...
		var me = this, propName = record.get(me.nameField), val = record
				.get(me.valueField), editor = me.customEditors[propName]
		dsField = "";
		...
		if (editor) {
			if (!(editor instanceof Ext.grid.CellEditor)) {
				...
				editor = me.customEditors[propName] = Ext.create(
						'Ext.grid.CellEditor', {
							field : editor
						});
			}
