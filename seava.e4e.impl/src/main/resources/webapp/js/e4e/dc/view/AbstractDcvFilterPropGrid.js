 ...
 */
Ext.define("e4e.dc.view.AbstractDcvFilterPropGrid",
		{
			extend : "e4e.dc.view.AbstractDc_PropGrid",
			...
			_getBuilder_ : function() {
				if (this._builder_ == null) {
					this._builder_ = new e4e.dc.view.DcvFilterPropGridBuilder({
						dcv : this
					});
				}
			...
			/**
			 * The parameters model is not part of a store, so we have listen to
			 * changes made to the model through the `parameterValueChanged`
			 * event raised by the data-control. Changes to the parameters model
			 * should be done through the setParamValue method of the
			 * data-control in order to be listened and picked-up to refresh the
