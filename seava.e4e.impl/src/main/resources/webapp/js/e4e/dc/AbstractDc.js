...
Ext
		.define(
				"e4e.dc.AbstractDc",
				{
					...
					/**
					 * Action instances. Used to create control widgets in UI to
					 * be triggered by user, mainly toolbar items.
					 */
					actions : null,

					/**
					 * Executable functions. Implements workers for API methods
					 * of a data-control.
					 */
					commands : null,
					...
					/**
					 * Filter model instance. Has the same signature as the data
					 * model instance
					 * 
					 * @type Ext.data.Model
					 */
					...
					recordModel : "",
					/**
					 * Filter model signature - filter constructor. Defaults to
					 * recordModel if not specified.
					 * 
					 * @type Ext.data.Model
					 */
					filterModel : null,
					...
					/**
					 * Children data-controls, similar to a `HasMany`
					 * association.
					 */
					children : null,

					/**
					 * Parent data-control, similar to a `BelongsTo`
					 * association.
					 * 
					...
					constructor : function(config) {
						...
						this.dcState = Ext.create("e4e.dc.DcState");
						this.children = [];
						this.selectedRecords = [];
						var _rm = Ext.isFunction(this.recordModel) ? this.recordModel.$className
								: this.recordModel;
						this.dsName = _rm.substring(_rm.lastIndexOf('.') + 1,
								_rm.length);
					...
					_setup_ : function() {
						...
						this.actionNames = e4e.dc.DcActionsFactory
								.actionNames();
						this.commandNames = e4e.dc.DcCommandFactory
								.commandNames();

						this.actions = e4e.dc.DcActionsFactory.createActions(
								this, this.actionNames);
						this.commands = e4e.dc.DcCommandFactory.createCommands(
								this, this.commandNames
										.concat(this.actionNames));
					...
					_registerListeners_ : function() {
						...
						this.mon(this.store, "beforeload",
								this.onStore_beforeload, this);
						this.mon(this.store, "update", this.onStore_update,
								this);
						this.mon(this.store, "datachanged",
								this.onStore_datachanged, this);
						this.mon(this.store, "changes_rejected",
								this.onStore_datachanged, this);

						/* after the store is loaded apply an initial selection */
						if (this.afterStoreLoadDoDefaultSelection) {
							this.mon(this.store, "load", this.onStore_load,
									this);
						}

						/* invoke the action state update whenever necessary */
						this.mon(this, "recordChange", this.requestStateUpdate,
								this);
						//
						this.mon(this, "selectionChange",
								this.requestStateUpdate, this);
						this.mon(this, "stateUpdateRequest",
								this.updateDcState, this, {
									buffer : 100
								});
					...
					/**
					 * Enter query. Fire the onEnterQuery event which should be
					 * listened by filter forms to acquire focus.
					 * 
					 */
					...
					/**
					 * Copy the current record reset its ID and make it current
					 * record ready to be edited.
					 */
					...
					/**
					 * Start edit process. Fires onEditIn event which Edit forms
					 * should listen to in order to acquire focus. Used for
					 * data-controls which are not multi edit and work with a
					 * grid/form pairs.
					 */
					...
					/**
					 * Finish edit process. Fires onEditOut event which grid
					 * lists should listen to in order to acquire focus. Used
					 * for data-controls which are not multi edit and work with
					 * a grid/form pairs.
					 */
					...
					/**
					 * Filter validator. It should get interceptor functions
					 * chain injected by the filter forms.
					 */
					...
					/**
					 * Record validator. It should get interceptor functions
					 * chain injected by the editor forms.
					 */
					...
					getRecordState : function() {
						if (this.record)
							return (this.isCurrentRecordDirty()) ? 'dirty'
									: 'clean';
						else
							return null;
					},
					...
					/**
					 * Update the enabled/disabled states of the actions.
					 * Delegate the work to the states manager.
					 */
					...
					isCurrentRecordDirty : function() {
						if (this.record != null
								&& (this.record.dirty || this.record.phantom)) {
							return true;
						}
						return false;
					},
					...
					/**
					 * Returns true if the data-control is dirty i.e either some
					 * the own records or any child
					 */
					isDirty : function() {
					...
					/**
					 * Helper function which packs the necessary information for
					 * a query. It is used in queryCommand, and export and print
					 * windows.
					 * 
					 * @return {}
					 */
					buildRequestParamsForQuery : function() {
						...
						var _p = {};
						_p[Main.requestParam.FILTER] = Ext
								.encode(this.filter.data);
						_p[Main.requestParam.ADVANCED_FILTER] = Ext
								.encode(this.advancedFilter || []);
						if (this.params != null) {
							_p[Main.requestParam.PARAMS] = Ext
					...
					/**
					 * Publish a message to notify subscribers. Used by a frame
					 * to write published messages in its notifications area (
					 * status bar )
					 */
					...
					info : function(message, trlGroup, params) {
						this.message(Main.msgType.INFO, message, trlGroup,
								params);
					},
					...
					warning : function(message, trlGroup, params) {
						this.message(Main.msgType.WARNING, message, trlGroup,
								params);
					},
					...
					error : function(message, trlGroup, params) {
						this.message(Main.msgType.ERROR, message, trlGroup,
								params);
					...
					/**
					 * Set filter property value. As currently the filter model
					 * is not part of a store, changes to properties must be
					 * fired so that filter views bound to the filter model can
					 * be notified to refresh their fields values.
					 * 
					 * In some future release it is likely to be enhanced so
					 * that filters can be saved and reused.
					 * 
					 * @param {String}
					...
					setFilterValue : function(property, newValue, silent,
						...
						if (oldValue != newValue) {
							...
							if (!(silent === true)) {
								this
										.fireEvent("filterValueChanged", this,
					...
					setParamValue : function(property, newValue, silent) {
						...
						if (ov != newValue) {
							...
							if (silent !== true) {
								this.fireEvent("parameterValueChanged", this,
										property, ov, newValue);
							}
						}
					},
					...
					/**
					 * Template method to override with instance specific logic
					...
					/**
					 * Set the given record as the current working record. If
					 * the selectIt parameter is true make it the selection.
					 * 
					...
					setRecord : function(p, selectIt, eOpts) {
						...
						if (!this.multiEdit) {
							if (this.isDirty()) {
								this.error(Main.msg.DIRTY_DATA_FOUND,
										Main.msgType.ERROR);
								return false;
							}
						}
					...
					setSelectedRecords : function(recArray, eOpts) {
						...
						if (this.selectedRecords !== recArray) {
							...
							if (this.record != null) {
								...
								if (!found) {
									this.setRecord(this.selectedRecords[0],
											false);
								}
							} else {
					 ...
					 */
					getParent : function() {
						return (this.dcContext) ? this.dcContext.parentDc
								: null;
					},
					...
					proxyException : function(proxy, response, operation, eOpts) {
						if (operation && operation.action == "destroy"
								&& !this.multiEdit) {
					...
					onAjaxRequestSuccess : function(response, options) {
						...
						if (o.action) {
							...
							if (o.action == "doService") {
								this.afterDoServiceSuccess(response,
										o.serviceName, o.specs);
							}
							if (o.action == "doServiceFilter") {
								this.afterDoServiceFilterSuccess(response,
										o.serviceName, o.specs);
							}
						}
					},
					...
					onAjaxRequestFailure : function(response, options) {
						...
						if (o.action) {
							...
							if (o.action == "doService") {
								this.afterDoServiceFailure(response,
										o.serviceName, o.specs);
							}
							if (o.action == "doServiceFilter") {
								this.afterDoServiceFilterFailure(response,
										o.serviceName, o.specs);
							}
						}
					},
