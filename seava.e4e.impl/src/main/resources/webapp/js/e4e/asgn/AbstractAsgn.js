...
Ext.define("e4e.asgn.AbstractAsgn", {
	...
	doSetupImpl : function() {
		Ext.Ajax.request({
			...
			url : Main.urlAsgn + "/" + this.dsName + ".json?"
					+ Main.requestParam.ACTION + "=" + Main.asgnAction.SETUP
		});
	},
	...
	doQueryLeftImpl : function() {
		...
		if (this.filter.left.field) {
			...
			this.storeLeft.proxy.extraParams[Main.requestParam.FILTER] = Ext
					.encode(data);
		}
	...
	doQueryRightImpl : function() {
		...
		if (this.filter.right.field) {
			...
			this.storeRight.proxy.extraParams[Main.requestParam.FILTER] = Ext
					.encode(data);
		}
	...
	doSaveImpl : function() {
		Ext.Ajax.request({
			...
			scope : this,
			url : Main.urlAsgn + "/" + this.dsName + ".json?"
					+ Main.requestParam.ACTION + "=" + Main.asgnAction.SAVE,
			options : {
				action : "doSave",
				fnSuccess : null,
				fnSuccessScope : null,
	...
	doResetImpl : function() {
		Ext.Ajax.request({
			...
			scope : this,
			url : Main.urlAsgn + "/" + this.dsName + ".json?"
					+ Main.requestParam.ACTION + "=" + Main.asgnAction.RESET,
			options : {
				action : "doReset",
				fnSuccess : null,
				fnSuccessScope : null,
	...
	doCleanupImpl : function() {
		Ext.Ajax.request({
			...
			scope : this,
			url : Main.urlAsgn + "/" + this.dsName + ".json?"
					+ Main.requestParam.ACTION + "=" + Main.asgnAction.CLEANUP
		});
	},
